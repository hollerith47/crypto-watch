...
const StarIcon = ({coinId}) => {
    ...
    useEffect(() => {
        if (window.location.coinList){
            let favList = window.localStorage.coinList.split(',');
            if (favList.includes(coinId)) setLike(true)
            else setLike(false)
        }
    }, [coinId]);
    ...
    return (
        <>
            <img onClick={() =>idChecker(coinId)} src={like ? "./assets/star-full.svg" : "./assets/star-empty.svg"} alt="star-icon"/>
        </>
    );
};
